...
func (r *supabasePlaylistRepo) AddTrack(ctx context.Context, track *entity.Track) error {
	...
	if _, err := r.db.Exec(ctx, q, track.PlaylistID, track.ID); err != nil {
...
func (r *supabasePlaylistRepo) ReorderTracks(ctx context.Context, playlistID string, orderedTrackIDs []string) error {
	tx, err := r.db.Begin(ctx)
	if err != nil {
		return fmt.Errorf("playlist_repo: reorder begin: %w", err)
	}
	...
	if _, err := tx.Exec(ctx,
		...
		return fmt.Errorf("playlist_repo: reorder offset: %w", err)
	}
	...
	for i, trackID := range orderedTrackIDs {
		...
		if err != nil {
			return fmt.Errorf("playlist_repo: reorder set: %w", err)
		}
		if tag.RowsAffected() == 0 {
			return fmt.Errorf("playlist_repo: track %s not in playlist %s", trackID, playlistID)
		}
	...
	if err := tx.Commit(ctx); err != nil {
		return fmt.Errorf("playlist_repo: reorder commit: %w", err)
	}
	return nil
}
